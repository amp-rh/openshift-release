...
typeset FA__CNV__SHARED_STORAGE_CLASS="${FA__CNV__SHARED_STORAGE_CLASS:-ibm-spectrum-scale-cnv}"
typeset FA__CNV__TEST_NAMESPACE="${FA__CNV__TEST_NAMESPACE:-cnv-snapshots-test}"
typeset FA__CNV__VM_NAME="${FA__CNV__VM_NAME:-test-snapshot-vm}"
typeset FA__CNV__SNAPSHOT_NAME="${FA__CNV__SNAPSHOT_NAME:-test-vm-snapshot}"
typeset FA__CNV__RESTORE_VM_NAME="${FA__CNV__RESTORE_VM_NAME:-restored-vm}"
typeset FA__CNV__VM_CPU_REQUEST="${FA__CNV__VM_CPU_REQUEST:-1}"
typeset FA__CNV__VM_MEMORY_REQUEST="${FA__CNV__VM_MEMORY_REQUEST:-1Gi}"
typeset FA__CNV__VM_SNAPSHOT_TIMEOUT="${FA__CNV__VM_SNAPSHOT_TIMEOUT:-25m}"
...
if [[ "${snapshotClasses}" -eq 0 ]]; then
  if ! {
    oc create -f - --dry-run=client -o yaml --save-config 0<<'YAML' |
...
then
  if oc wait vmsnapshot "${FA__CNV__SNAPSHOT_NAME}" -n "${FA__CNV__TEST_NAMESPACE}" --for=condition=Ready --timeout="${FA__CNV__VM_SNAPSHOT_TIMEOUT}"; then
    ...
    if ! oc get vmsnapshot "${FA__CNV__SNAPSHOT_NAME}" -n "${FA__CNV__TEST_NAMESPACE}" -o yaml --ignore-not-found; then
      true
    fi
...
then
  if oc wait vmrestore "${FA__CNV__RESTORE_VM_NAME}-restore" -n "${FA__CNV__TEST_NAMESPACE}" --for=condition=Ready --timeout="${FA__CNV__VM_SNAPSHOT_TIMEOUT}"; then
    ...
    if ! oc get vmrestore "${FA__CNV__RESTORE_VM_NAME}-restore" -n "${FA__CNV__TEST_NAMESPACE}" -o yaml --ignore-not-found; then
      true
    fi
